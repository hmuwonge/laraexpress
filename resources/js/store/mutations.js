export default {
    ...
    getAllCampaign(state, data) {
        ...
        for (let i = 0; i < campaign.length; i++) {
            let found = state.all_campaigns.some(
                (el) => el.id === campaign[i].id
            );
    ...
    getCampaignProducts(state, data) {
        ...
        let index = campaigns.findIndex(
            (campaign) => campaign.slug == data.slug
        );
    ...
    getRemoveWishlist(state, data) {
        var index = state.wishlist_products.findIndex((c) => c.id == data.id);
    ...
    getRemoveCompare(state, data) {
        var index = state.compare_products.findIndex((c) => c.id == data.id);
    ...
    getCategoryProducts(state, data) {
        var index = state.category_products.findIndex(
            (c) => c.slug == data.slug
        );
        if (index > -1) {
    ...
    getBrandProducts(state, data) {
        var index = state.brand_products.findIndex((c) => c.slug == data.slug);
        if (index > -1) {
    ...
    getWalletRecharges(state, data) {
        for (let i = 0; i < data.data.length; i++) {
            let found = state.wallet_recharges.some(
    ...
    getRemoveFollower(state, data) {
        var index = state.shop_follwer.findIndex(
            (c) => c.seller_id == data.seller_id
        );
        if (index > -1) {
    ...
    setFollowedSellers(state, data) {
        for (let i = 0; i < data.length; i++) {
            let found = state.followedSellers.some(
                (el) => el.id === data[i].id
            );
            if (!found) {
    ...
    setSellerProducts(state, data) {
        ...
        let index = seller_products.findIndex(
            (seller) => seller.slug == data.slug
        );
